//go:build all || outerloop || outerloop_scan_multiple_apps
...
func TestOuterloopScanSupplychainMultipleApps(t *testing.T) {
	...
	updateTap := features.New("update-tap-full-supplychainbasic").
		Assess("update-package", func(ctx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			tapValuesSchema.TapGui.ServiceType = "LoadBalancer"
	...
	testenv.Test(t,
		...
		deployLenientScanPolicy,
		deployBuildPacksPipeline,
		deployBuildPackWorkloads,
		verifyBuildPackWorkloadsGitrepoStatus,
		verifyBuildPackWorkloadsTestTaskrunStatus,
